...
public final class Model {
    ...
    private final int TAMANHO = 2; // tamanho inicial e crescimento

    private final int MAX_INDEX_X = WIDTH / SCALE;
    private final int MAX_INDEX_Y = HEIGHT / SCALE;

    private int squaresToGrow;
    private int macaComidas = 0;
    private Direction direction = Direction.DOWN; //Direção incial da cobrinha
  ...
  //      initSounds();
	...
	//            stopMusic();
	//            playGameOverSound();
	            data[difficulty] = Math.max(macaComidas, data[difficulty]);
	            view.update(difficulties[difficulty], macaComidas, data[difficulty]);
	        ...
	        view.updateView(snakeBody, apple, difficulties[difficulty], data[difficulty], macaComidas);
